...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GHR);
    ...
    if (!settings.contains("nAnonymizeGHRAmount"))
        settings.setValue("nAnonymizeGHRAmount", 1000);

    nPrivateSendRounds = settings.value("nPrivateSendRounds").toLongLong();
    nAnonymizeGHRAmount = settings.value("nAnonymizeGHRAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGHRAmount"))
        SoftSetArg("-anonymizeghramount", settings.value("nAnonymizeGHRAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ghr.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGHRAmount:
            return QVariant(nAnonymizeGHRAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case PrivateSendRounds:
            ...
            break;
        case AnonymizeGHRAmount:
            nAnonymizeGHRAmount = value.toInt();
            settings.setValue("nAnonymizeGHRAmount", nAnonymizeGHRAmount);
            emit anonymizeGHRAmountChanged(nAnonymizeGHRAmount);
            break;
        case CoinControlFeatures:
